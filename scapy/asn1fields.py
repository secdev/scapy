...
class ASN1F_field(ASN1F_element):
    ...
    def extract_packet(self,
                       cls,  # type: ASN1Packet_metaclass
                       s,  # type: bytes
                       ):
        # type: (...) -> Tuple[BasePacket, bytes]
        if len(s) > 0:
            try:
                c = cls(s)
...
class ASN1F_SEQUENCE(ASN1F_field):
    ...
    def m2i(self, pkt, s):
        # type: (Any, bytes) -> Tuple[List, bytes]
        """
...
class ASN1F_SEQUENCE_OF(ASN1F_field):
    ...
    def m2i(self, pkt, s):
        # type: (Any, bytes) -> Tuple[List[BasePacket], bytes]
        diff_tag, s = BER_tagging_dec(s, hidden_tag=self.ASN1_tag,
                                      implicit_tag=self.implicit_tag,
                                      explicit_tag=self.explicit_tag,
                                      safe=self.flexible_tag)
...
class ASN1F_BIT_STRING_ENCAPS(ASN1F_BIT_STRING):
    ...
    def m2i(self, pkt, s):
        # type: (BasePacket, bytes) -> Tuple[Any, bytes]
