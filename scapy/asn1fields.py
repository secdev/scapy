...
def _oer():
    # type: () -> Any
    from scapy.contrib import oer as _m
    return _m


def _uper():
    # type: () -> Any
    from scapy.contrib import uper as _m
    return _m
...
class ASN1F_field(ASN1F_element, Generic[_I, _A]):
    ...
    def _codec_kwargs(self, pkt):
        ...
        # OER/UPER need extra constraints on every enc/dec call.
        if pkt.ASN1_codec == ASN1_Codecs.PER:
            return self._uper_codec_kwargs()
        kwargs = {"size_len": self.size_len}  # type: Dict[str, Any]
        if pkt.ASN1_codec == ASN1_Codecs.OER:
            kwargs["size_len"] = self.size_len or 0
            if self.oer_unsigned:
                kwargs["oer_unsigned"] = self.oer_unsigned
        return kwargs

    def _use_object_enc(self, pkt, item):
        # type: (ASN1_Packet, ASN1_Object[Any]) -> bool
        # BER/LDAP: item.enc() when size_len is unset. PER/OER constraints
        # must go through codec.enc(**kwargs).
        if pkt.ASN1_codec == ASN1_Codecs.PER:
            return False
        if pkt.ASN1_codec == ASN1_Codecs.OER:
            return self.size_len is None and not self.oer_unsigned
    ...
    def _encode_item(self, pkt, item):
        ...
        if isinstance(item, ASN1_Object):
            ...
            # the type codec so the universal tag/length are applied.
...
class ASN1F_SEQUENCE(ASN1F_field[List[Any], List[Any]]):
    ...
    def _m2i_per(self, pkt, s):
        # type: (Any, bytes) -> Tuple[Any, bytes]
        dec = _uper().UPER_Decoder(s)
        self._uper_dissect_from_decoder(pkt, dec)
        if _uper().UPER_has_unexpected_remainder(dec):
            raise _uper().UPER_Decoding_Error(
...
List[ASN1_Object[Any]]]):
    ...
    def _uper_count_enc(self, enc, count):
        ...
        if self.uper_min is not None and self.uper_max is not None:
            _uper().UPER_constrained_int_enc(count, self.uper_min, self.uper_max, enc=enc)
        else:
        ...
        if self.uper_min is not None and self.uper_max is not None:
            ...
            return cast(
                ...
                dec.read_non_negative_binary_integer(
                    _uper().UPER_bits_for_range(size),
                ) + self.uper_min,
            )
    ...
    def _extract_packet_from_decoder(self, dec, pkt):
        # type: (Any, ASN1_Packet) -> Tuple[Any, bytes]
    ...
    def m2i(self,
        ...
        if pkt.ASN1_codec == ASN1_Codecs.OER:
            ...
            count, s = _oer().OER_unsigned_integer_dec(s)
            lst = []
            for _ in range(count):
        ...
        if pkt.ASN1_codec == ASN1_Codecs.PER:
            ...
            for _ in range(count):
                ...
                if c:
                    lst.append(c)
            if _uper().UPER_has_unexpected_remainder(dec):
                raise _uper().UPER_Decoding_Error("unexpected remainder",
                                          remaining=dec.remaining())
            return lst, b""
        s = self._apply_tagging_dec(s, pkt)
        codec = self.ASN1_tag.get_codec(pkt.ASN1_codec)
    ...
    def build(self, pkt):
        ...
        if isinstance(val, ASN1_Object) and \
            ...
            s = b""
            if pkt.ASN1_codec == ASN1_Codecs.OER:
                s = _oer().OER_unsigned_integer_enc(0)
            ...
            if pkt.ASN1_codec == ASN1_Codecs.PER:
                ...
                if pkt.ASN1_codec == ASN1_Codecs.OER:
                    s = _oer().OER_unsigned_integer_enc(len(val)) + s
            else:
                # BER/OER: element fields may carry implicit/explicit tags;
                # i2m matches m2i()/fld.m2i().
...
class ASN1F_optional(ASN1F_element):
    ...
    def is_empty(self, pkt):
        # type: (ASN1_Packet) -> bool
        # Delegate to the wrapped field (e.g. SEQUENCE checks children).
...
class ASN1F_CHOICE(ASN1F_field[_CHOICE_T, ASN1_Object[Any]]):
    ...
    def m2i_from_decoder(self, pkt, dec):
        # type: (ASN1_Packet, Any) -> ASN1_Object[Any]
        if self.uper_extensible:
            if dec.read_bit():
                raise _uper().UPER_Decoding_Error(
        ...
        if len(self.choice_order) > 1:
            index, _ = _uper().UPER_choice_index_dec(b"", len(self.choice_order), dec=dec)
        else:
            index = 0
    ...
    def _uper_encode_into(self, enc, pkt, value=None):
        ...
        if self.uper_extensible:
            enc.append_bit(0)
        if len(self.choice_order) > 1:
            _uper().UPER_choice_index_enc(index, len(self.choice_order), enc=enc)
        choice = self._choice_for_index(index)
        if hasattr(choice, "ASN1_root"):
        ...
        if x is None:
            ...
            if pkt.ASN1_codec == ASN1_Codecs.OER:
                ...
                if alt_tag is not None:
                    s = _oer().OER_tag_enc(alt_tag & 0x3f, alt_tag & 0xc0) + s
...
class ASN1F_PACKET(ASN1F_field['ASN1_Packet', Optional['ASN1_Packet']]):
    ...
    def m2i_from_decoder(self, pkt, dec):
        # type: (ASN1_Packet, Any) -> Optional[ASN1_Packet]
