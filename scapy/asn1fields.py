...
class ASN1F_field(ASN1F_element, Generic[_I, _A]):
    ...
    def _codec_kwargs(self, size_len=None):
        ...
        if getattr(self, "oer_unsigned", False):
    ...
    def m2i(self, pkt, s):
        ...
        if pkt.ASN1_codec == ASN1_Codecs.PER:
            ...
            if pkt.ASN1_codec == ASN1_Codecs.OER:
                codec_kwargs = dict(codec_kwargs)
                codec_kwargs["size_len"] = self.size_len or 0
    ...
    def i2m(self, pkt, x):
        ...
        if isinstance(x, ASN1_Object):
            ...
            elif pkt.ASN1_codec == ASN1_Codecs.OER:
                s = codec.enc(x, size_len=size_len)
            else:
                s = codec.enc(x, **self._codec_kwargs(size_len))
    ...
    def _encode_item(self, pkt, item):
        ...
        return codec.enc(item, **self._codec_kwargs(size_len))
...
class ASN1F_CHOICE(ASN1F_field[_CHOICE_T, ASN1_Object[Any]]):
    ...
    def _m2i_oer(self, pkt, s):
        ...
        _, s = self._tagging_dec(
            pkt, s,
            hidden_tag=self.ASN1_tag, explicit_tag=self.explicit_tag,
        )
