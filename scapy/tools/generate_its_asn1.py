...
def root_members(members: List[Any]) -> List[Dict[str, Any]]:
    """Members of the extension root, the ones a codec encodes in place.

    asn1tools gives an extension marker as None, so the members that follow
    it, up to the marker closing the additions, are extension additions. They
    are encoded out of the root, behind their own preamble, and taking them
    for root members would shift every bit behind them.
    """
    root: List[Any] = []
    additions = False
    for member in members:
        if member is None:
            additions = not additions
        elif not additions:
            root.append(member)
    return flatten_members(root)
...
class ITSASN1Generator:
    ...
    def _min_size(self, desc: Dict[str, Any]) -> int:
            ...
            for module, data in self.spec.items():
                for type_name, desc in list(data.get("types", {}).items()):
                    if desc.get("type") not in ("SEQUENCE", "CHOICE"):
                        continue
                    for member in root_members(desc.get("members", [])):
    ...
    def _primitive_field(self, member_name: str, desc: Dict[str, Any], tag_args: str, in_choice: bool, optional: bool = False) -> str:
        ...
        if optional:
            ...
            # A sized string is encoded with that many units, so an empty
            # default would not even build.
            size = self._min_size(desc)
            default = b"\x00" * size if base == "OCTET STRING" else "0" * size
        elif base == "BIT STRING":
            default = "0" * self._min_size(desc)
        elif base == "INTEGER":
        ...
        if cls == "ASN1F_BIT_STRING" and not optional:
            # ASN1F_FLAGS already reads its default as a bit string.
            extra += ", default_readable=False"
    ...
    def _choice_root(
        ...
        members = root_members(desc.get("members", []))
        alts: List[str] = []
        for member in members:
    ...
    def generate(self) -> str:
        ...
        lines.append("")
        lines.append("# Registers the PER codec that ASN1_Codecs.PER refers to")
        lines.append("import scapy.contrib.uper  # noqa: F401")
        lines.append("")
