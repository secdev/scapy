...
if conf.use_winpcapy:
    ...
    # Part of the code from https://github.com/phaethon/scapy translated to
    # python2.X
    try:
        ...
        if b"winpcap" in version.lower():
            if os.path.exists(NPCAP_PATH + "\\wpcap.dll"):
                warning(
                    "Winpcap is installed over Npcap. Will use Winpcap (see 'Winpcap/Npcap conflicts' in scapy's docs)",
                    onlyOnce=True)
            ...
            while p:
                ...
                while a:
                    if a.contents.addr.contents.sa_family == socket.AF_INET:
                        ...
                        if_raw_addr = b"".join(chb(x)
                                               for x in val.contents.sin_addr[:4])
                        if if_raw_addr != b'\x00\x00\x00\x00':
                            conf.cache_ipaddrs[plain_str(
                                p.contents.name)] = if_raw_addr
                    a = a.contents.next
                p = p.contents.next
            return conf.cache_ipaddrs.get(iff.pcap_name, None)
        finally:
            ...
            while p:
                ...
                while a:
                    if a.contents.addr.contents.sa_family == socket.AF_INET6:
                        ...
                        addr = inet_ntop(
                            socket.AF_INET6, b"".join(
                                chb(x) for x in val.contents.sin6_addr[:]))
    ...
    class _PcapWrapper_pypcap:
        ...
        def next(self):
            ...
            ts = self.header.contents.ts.tv_sec + \
                float(self.header.contents.ts.tv_usec) / 1000000
            pkt = b"".join(chb(i)
    ...
    class L3pcapSocket(L2pcapSocket):
        ...
        def send(self, x):
            # Makes send detects when it should add Loopback(), Dot11...
            # instead of Ether()
            ll = self.ins.datalink()
            if ll in conf.l2types:
                cls = conf.l2types[ll]
    ...
    if conf.use_pcap:
        ...
        if hasattr(pcap, "pcap"):  # python-pypcap
            ...
            class _PcapWrapper_pcapy:
                ...
                def fileno(self):
                    raise RuntimeError(
                        "%s has no fileno. Please report this bug." %
...
if conf.use_pcap and conf.use_dnet:
    class L3dnetSocket(SuperSocket):
        ...
        def __init__(self, type=ETH_P_ALL, promisc=None,
                     filter=None, iface=None, nofilter=0):
            self.iflist = {}
            self.intf = dnet.intf()
            if iface is None:
                iface = conf.iface
