...
from scapy.base_classes import BasePacket, Gen, Packet_metaclass, SetGen
from scapy.config import conf
from scapy.consts import PYX
from scapy.error import Scapy_Exception, log_runtime
from scapy.fields import (BitField, ConditionalField, Emph, EnumField,
                          FlagsField, MultiEnumField, PacketListField,
                          StrField)
from scapy.utils import colgen, get_temp_file, import_hexcap, tex_escape
...
class Packet(BasePacket):
    ...
    def upper_bonds(self):
        for fval, upper in self.payload_guess:
            print "%-20s  %s" % (upper.__name__, ", ".join(
                "%-12s" % ("%s=%r" % i) for i in fval.iteritems()))

    @classmethod
    def lower_bonds(self):
        for lower, fval in self._overload_fields.iteritems():
            print "%-20s  %s" % (lower.__name__, ", ".join(
                "%-12s" % ("%s=%r" % i) for i in fval.iteritems()))
    ...
    def __init__(
            self, _pkt="", post_transform=None, _internal=0, _underlayer=None,
            **fields):
        self.time = time.time()
        self.sent_time = None
    ...
    def add_payload(self, payload):
        if payload is None:
            ...
            if isinstance(payload, Packet):
                ...
                raise TypeError(
                        "payload must be either 'Packet' or 'str', " +
                        "not [%s]" % repr(payload))
    ...
    def setfieldval(self, attr, val):
        if attr in self.default_fields:
            ...
            if fld is None:
                # use pep8 recommandation to modify the function
                # (https://www.python.org/dev/peps/pep-0008/#id50)
                def any2i(x, y): return y
            else:
                def any2i(x, y): return fld.any2i(x, y)
        ...
        for f in self.fields_desc:
            ...
            if isinstance(val, RawVal):
                ...
                if field_pos_list is not None:
                    field_pos_list.append((f.name, sval.encode(
                        "string_escape"), len(p), len(sval)))
            else:
...
#        backcolor=makecol(0.376, 0.729, 0.525, 1.0)
        ...
        while t:
            ...
            canvas.stroke(ptbb.path(), [
                pyx.color.rgb.black, pyx.deco.filled([bkcol])])
            canvas.insert(pt)
    ...
    def do_dissect(self, s):
        """
        Perform the dissection of the layer

        :string str s: the raw layer
        """
        raw = s
        self.raw_packet_cache_fields = {}
        for f in self.fields_desc:
    ...
    def hide_defaults(self):
        ...
        """
        # use .items(): self.fields is modified in the loop
    ...
    def hashret(self):
        """
        DEV: returns a string that has the same value for a request and its
        answer.
        """
    ...
    def haslayer(self, cls):
        """
        true if self has a layer that is an instance of cls. Superseded by "cls
        in self" syntax.
        """
    ...
    def getlayer(self, cls, nb=1, _track=None):
        ...
        if type(cls) is str and "." in cls:
            ...
            ccls, fld = cls, None
        if cls is None or \
                self.__class__ == cls or self.__class__.__name__ == ccls:
            if nb == 1:
                if fld is None:
                    return self
                else:
    ...
    def __contains__(self, cls):
        """
        "cls in self"
        returns true if self has a layer which is an instance of cls.
        """
...
"""
        ...
        while "{" in fmt:
            ...
            if k < 0:
                raise Scapy_Exception(
                        "Bad condition in format string: [%s] (read sprintf " +
                        "doc!)" % cond)
        ...
        while "%" in fmt:
            ...
            try:
                ...
                if fld == "time":
                    ...
                    if num > 1:
                        val = self.payload.sprintf("%%%s, %s:%s.%s%%" % (
                            f, cls, num-1, fld), relax)
                        f = "s"
    ...
    def mysummary(self):
        ...
        Only one mysummary() is used in a whole packet summary: the one of the
        upper layer, except if a mysummary() also returns (as a couple) a list
        of layers whose mysummary() must be called if they are present.
        """
        return ""
    ...
    def libnet(self):
        """
        Not ready yet. Should give the necessary C code that interfaces with
        libnet to recreate the packet
        """
...
def ls(obj=None, case_sensitive=False, verbose=False):
    ...
    if obj is None or is_string:
        if obj is None:
            ...
            all_layers = sorted(
                    (layer for layer in conf.layers if (pattern.search(
                            layer.__name__ or '') or pattern.search(
                                layer.name or ''))), key=lambda x: x.__name__)
        ...
        if (isinstance(obj, type) and issubclass(obj, Packet)) or is_pkt:
            ...
            print "Not a packet class or name. Type 'ls()' to list packet " + \
                    "classes."
...
def fuzz(p, _inplace=0):
    """
    Transform a layer into a fuzzy layer by replacing some default values by
    random objects
    """
    if not _inplace:
        p = p.copy()
