...
def rdcandump(filename, count=-1, interface=None):
    # type: (BytesIO, int, Union[List[str], None, str]) -> NoReturn
...
class CandumpReader:
    ...
    def __init__(self, filename, interface=None):
        # type: (BytesIO, Optional[str]) -> None
    ...
    @staticmethod
    def open(filename):
        # type: (BytesIO) -> Tuple[str, BytesIO]
