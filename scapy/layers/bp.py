...
class BP(Packet):
    ...
    fields_desc = [ ByteField('version', 0),
                    ...
                    SDNV2('LT', 0),
                    SDNV2('DL', 0),
                    ConditionalField(SDNV2("FO", 0), lambda Packet: (Packet.ProcFlags & 0x01)),
                    ConditionalField(SDNV2("ADUL", 0), lambda Packet: (Packet.ProcFlags & 0x01)),
                  ]
    def mysummary(self):
        tmp = ""
        ...
        #return self.sprintf("BP(%version%) flags(%ProcFlags%) len(%BlockLen%) d[%DSO%,%DSSO%] s[%SSO%, %SSSO%] r[%RTSO%, %RTSSO%] c[%CSO%, %CSSO%] ct[%CT%] lt[%LT%] fo[%FO%] adul[%ADUL%]"), [LTP]
    
class BPBLOCK(Packet):
    fields_desc = [ ByteField('Type', 0),
                    SDNV2('ProcFlags', 0),
                    SDNV2('BlockLen', 0),
                    StrLenField("the_varfield", "the_default_value", length_from = lambda pkt: pkt.BlockLen)
                  ]
    def mysummary(self):
        tmp = ""
