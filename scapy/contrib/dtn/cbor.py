...
class CBORIntOrText(CBORBase):
    def getfield(self, pkt, s):
        ...
        if major_type == 0: return CBORInteger.get_value(add_info, s)
        if major_type == 3: return CBORStringBase.get_value(add_info, s)
...
class CBORAny(CBORBase):
    def getfield(self, pkt, s):
        ...
        if major_type == 0: return CBORInteger.get_value(add_info, s)
        if major_type in [2, 3]: return CBORStringBase.get_value(add_info, s)
        if major_type == 7: return CBORStopCode.get_value(add_info, s)
