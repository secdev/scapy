...
def _supported_id_numbers(socket, timeout, service_class, id_name, verbose):
    ...
    Args:
        socket: is the ISOTPSocket, over which the OBD-Services communicate.
                the id 0x7df acts as a broadcast address for all obd-supporting
                 ECUs.
        timeout: only required for the OBD Simulator, since it might tell it
                 supports a PID, while it actually doesn't and won't respond to
                  this PID.
                 If this happens with a real ECU, it is an implementation
                 error.
    ...
    This method sends a query message via a ISOTPSocket, which will be
    responded by the ECUs with a message containing Bits, representing whether
    a PID is supported by the vehicle's protocol implementation or not.
    The first Message has the PID 0x00 and contains 32 Bits, which indicate by
    their index and value, which PIDs are supported.
    If  the PID 0x20 is supported, that means, there are more supported PIDs
    within the next 32 PIDs, which will result
...
def _scan_id_service(socket, timeout, service_class, id_numbers, verbose):
    ...
    This method queries the specified id_numbers and stores their responses in
     a dictionary, which is then returned.
    """
...
def _scan_dtc_service(socket, timeout, service_class, verbose):
    """ Queries Diagnostic Trouble Code Parameters and stores their return
        value
    ...
    This method queries the specified Diagnostic Trouble Code Parameters and
    stores their responses in a dictionary, which is then returned.
    """
...
def obd_scan(socket, timeout=0.1, supported_ids=False, unsupported_ids=False,
             verbose=False):
    """ Scans for all accessible information of each commonly used OBD service
        classes and prints the results
    ...
    Args:
        ...
        supported_ids: specifies, whether to check for supported Parameter IDs.
                       The OBD-Protocol offers querying, which PIDs the
                       implemented ECUs support.
        unsupported_ids: specifies, whether to check for unsupported or hidden
                         Parameter IDs.
                         There is a possibility of PIDs answering, which are
                         addressed directly, but which are
                         not listed in the supported query response. We call
                         these PIDs unsupported PIDs, because
                         they are seemingly unsupported.
        verbose: specifies, whether the sr1()-method gives feedback or not and
                 turns.

    This method queries the Diagnostic Trouble Code Parameters and if selected,
     supported and/or unsupported PIDS and prints the results.
    """
    ...
    # Emission-related DTCs detected during current or last completed
    # driving cycle
    ...
    if supported_ids:
        ...
        supported[1] = _scan_id_service(socket, timeout, OBD_S01,
                                        supported_ids_s01, verbose)
        supported[6] = _scan_id_service(socket, timeout, OBD_S06,
                                        supported_ids_s06, verbose)
        supported[8] = _scan_id_service(socket, timeout, OBD_S08,
                                        supported_ids_s08, verbose)
        supported[9] = _scan_id_service(socket, timeout, OBD_S09,
                                        supported_ids_s09, verbose)
    ...
    # this option will slow down the test a lot, since it tests for
    # seemingly unsupported ids
    ...
    if unsupported_ids:
        ...
        # the unsupported id ranges are obtained by creating the compliment
        # set excluding 0
        ...
        unsupported[1] = _scan_id_service(socket, timeout, OBD_S01,
                                          unsupported_ids_s01, verbose)
        unsupported[6] = _scan_id_service(socket, timeout, OBD_S06,
                                          unsupported_ids_s06, verbose)
        unsupported[8] = _scan_id_service(socket, timeout, OBD_S08,
                                          unsupported_ids_s08, verbose)
        unsupported[9] = _scan_id_service(socket, timeout, OBD_S09,
                                          unsupported_ids_s09, verbose)
