...
def _open_sa_filtered_sock(sock, target_sa):
    ...
    ``j1939_pre_probe_flush`` mechanism handles those cases.
    ...
    try:
        ...
        j1939_log.debug("failed to create filtered rx socket: %s", ex)
        return sock, False
...
def j1939_resolve_probe_sock(sock, target_sa, reconnect_handler=None):
    ...
    When *reconnect_handler* is provided, it is called to create a fresh per-probe socket
    ...
    When *reconnect_handler* is None and *sock* is a live socket, the original
...
def j1939_scan_addr_claim(
    ...
    with (
        ...
        for _sa in src_addrs:
            ...
            req = J1939Request(
                req_pgn=J1939_PGN_ADDRESS_CLAIMED, dst=J1939_GLOBAL_ADDRESS, src=_sa
            )
            with j1939_get_sock(
                base_j_sock, src_addr=_sa, reconnect_handler=reconnect_handler
            ) as j_sock:
                j1939_log.debug(
                ...
                for _, rcv in ans:
                    ...
                    if rcv.pgn == J1939_PGN_ADDRESS_CLAIMED:
                        ...
                        if not force and noise_ids is not None and sa in noise_ids:
                            j1939_log.debug("addr_claim: suppressing noise SA=0x%02X", sa)
                            continue
                        if sa not in found:
                            found[sa] = []
                        found[sa].append(
        ...
        _extra = j1939_inter_probe_delay(bitrate, busload, 3, 8, listen_time)
        if _extra > 0.0:
            time.sleep(_extra)
...
def j1939_scan_ecu_id(
    ...
    with (
        j1939_get_sock(sock, include_tp_cm=True)
        if reconnect_handler is None
        else contextlib.nullcontext(sock)
    ) as base_j_sock:
        for _sa in src_addrs:
            ...
            req = J1939Request(
                req_pgn=J1939_PGN_ECU_ID, dst=J1939_GLOBAL_ADDRESS, src=_sa
            )
            with j1939_get_sock(
                base_j_sock, src_addr=_sa, reconnect_handler=reconnect_handler, include_tp_cm=True
            ) as j_sock:
                j1939_log.debug(
            ...
            for _, rcv in ans:
                ...
                if rcv.pgn == J1939_PGN_ECU_ID:
                    ...
                    if len(d) >= 8 and d[0] == J1939_TP_CTRL_BAM:
                        ...
                        if bam_pgn == J1939_PGN_ECU_ID:
                            matched = True
                if matched:
                    sa = rcv.src
                    if not force and noise_ids is not None and sa in noise_ids:
                        j1939_log.debug("ecu_id: suppressing noise SA=0x%02X", sa)
                        continue
    ...
    with (
        ...
        for da in scan_range:
            ...
            if not force and noise_ids is not None and da in noise_ids:
                j1939_log.debug("unicast: skipping noise DA=0x%02X", da)
                continue
    ...
    with (
        ...
        for da in scan_range:
            if stop_event is not None and stop_event.is_set():
                break
            if not force and noise_ids is not None and da in noise_ids:
                j1939_log.debug("rts_probe: skipping noise DA=0x%02X", da)
                continue
            ...
            with j1939_get_sock(
                ...
                for _sa in src_addrs:
                    ...
                    j1939_log.debug("rts_probe: probing DA=0x%02X from SA=0x%02X", _da, _sa)
                    rcv = j_sock.sr1(req, timeout=sniff_time, verbose=False)
                    if rcv is not None:
                        d = rcv.data if rcv.data else bytes(rcv.payload)
                        if d:
                            if rcv.pgn == J1939_PGN_TP_CM and d[0] in (
                                J1939_TP_CTRL_CTS,
                                J1939_TP_CTRL_ABORT,
                            ):
                                j1939_log.debug(
    ...
    with (
        ...
        if not skip_functional:
            for _sa in src_addrs:
                if stop_event is not None and stop_event.is_set():
                    break
                with j1939_get_sock(base_j_sock, src_addr=_sa, reconnect_handler=reconnect_handler) as j_sock:
                    j1939_log.debug(
        ...
        for da in scan_range:
            if stop_event is not None and stop_event.is_set():
                break
            if not force and noise_ids is not None and da in noise_ids:
                j1939_log.debug("uds: skipping noise DA=0x%02X", da)
                continue
    ...
    with (
        ...
        for da in scan_range:
            if stop_event is not None and stop_event.is_set():
                break
            if not force and noise_ids is not None and da in noise_ids:
                j1939_log.debug("xcp: skipping noise DA=0x%02X", da)
                continue
