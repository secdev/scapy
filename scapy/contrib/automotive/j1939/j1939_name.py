...
def j1939_request_name(
    ...
    req = J1939Request(req_pgn=PGN_ADDRESS_CLAIMED, dst=target_da)

    if target_da == J1939_GLOBAL_ADDRESS:
        # Broadcast request
        found: Dict[int, J1939_NAME] = {}
        with _get_j1939_sock(sock, src_addr=src_addr, reconnect=reconnect) as j_sock:
            log_j1939.debug(
                "j1939_request_name: broadcast request sent (CAN-ID=0x%08X)",
                _j1939_can_id(6, J1939_PF_REQUEST, J1939_GLOBAL_ADDRESS, src_addr),
            )
        ...
        with _get_j1939_sock(
            sock, src_addr=src_addr, reconnect=reconnect, target_sa=target_da
        ) as j_sock:
            log_j1939.debug(
                "j1939_request_name: unicast request sent to DA=0x%02X (CAN-ID=0x%08X)",
                target_da,
                _j1939_can_id(6, J1939_PF_REQUEST, target_da, src_addr),
            )
