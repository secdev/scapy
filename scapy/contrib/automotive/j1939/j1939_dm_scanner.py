...
def j1939_scan_dm_pgn(
    ...
    can_id = j1939_can_id(_DM_SCAN_PRIORITY, J1939_PF_REQUEST, target_da, src_addr)
    payload = struct.pack("<I", pgn)[:3]

    send_sock, rx_sock, close_rx = j1939_resolve_probe_sock(
        sock, target_da, reconnect_handler=reconnect_handler
    )
    ...
    def _rx(pkt):
        ...
        if j1939_pgn_from_fields(0, pf, ps) == pgn:
            j1939_log.debug("dm_scan: positive response SA=0x%02X PGN=0x%04X", sa, pgn)
        ...
        if pf == J1939_TP_CM_PF:
            ...
            if len(data) >= 8:
                ...
                if tp_pgn == pgn:
                    if (ctrl == J1939_TP_CTRL_BAM and ps == J1939_GLOBAL_ADDRESS) or \
                       (ctrl == J1939_TP_CTRL_RTS and ps == src_addr):
                        j1939_log.debug(
        ...
        if pf == J1939_PF_ACK:
            ...
            if data and data[0] == _ACK_CTRL_NACK:
                j1939_log.debug("dm_scan: NACK from SA=0x%02X PGN=0x%04X", sa, pgn)
                result.append(
    ...
    j1939_log.debug("dm_scan: timeout waiting for DA=0x%02X PGN=0x%04X", target_da, pgn)
    ...
    for i, dm_name in enumerate(dms):
        ...
        if i < num_pgns - 1:
            if reset_handler is not None:
                j1939_log.debug("dm_scan: calling reset_handler between probes")
                reset_handler()
            if reconnect_handler is not None:
                j1939_log.debug("dm_scan: calling reconnect_handler")
