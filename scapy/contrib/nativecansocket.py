...
class CANSocket(SuperSocket):
    ...
    def __init__(self, iface=None, receive_own_messages=False, can_filters=None, remove_padding=True):
        self.remove_padding = remove_padding
        self.iface = conf.contribs['NativeCANSocket']['iface'] if iface is None else iface
        ...
        try:
            ...
            Scapy_Exception("Could not modify receive own messages (%s)", exception)
    ...
    def recv(self, x=CAN_FRAME_SIZE):
        ...
        # need to change the byteoder of the first four bytes, required by the
        # underlaying linux CAN frame format
