...
def _sr1_rtrequest(pkt: Packet) -> List[Packet]:
    """
    Send/Receive a rtnetlink request safely.
    Root required for raw netlink sockets.
    Non-root fallback: returns empty list instead of crashing.
    """
    results: List[Packet] = []
    ...
    try:
        ...
        # Optional Linux-specific options
        try:
        ...
        try:
            ...
            log_loading.warning("[!] Non-root detected: RTNETLINK access denied, skipping.")
            return []
        ...
        while True:
            ...
            if not msgs:
                log_loading.warning("[!] Failed to read routes using RTNETLINK.")
                return []
        ...
        log_loading.warning("[!] Non-root detected: skipping RTNETLINK requests.")
        return []
    except Exception as e:
        # Catch any other exceptions and fallback gracefully
        try:
            log_loading.warning(f"[!] RTNETLINK failed: {e}")
        except Exception:
            pass
        return []
    finally:
